...
public class DeepSeekService implements AIService {
    ...
    @Value("${ai.providers.deepseek.base-url:https://api.deepseek.com}")
    private String baseUrl;

    @Value("${ai.providers.deepseek.model:deepseek-chat}")
    private String model;

    @Value("${ai.providers.deepseek.enabled:true}")
    ...
    public ChatResponse chat(ChatRequest request) {
        ...
        if (!enabled || apiKey == null || apiKey.isEmpty()) {
            throw new RuntimeException("DeepSeek service is not configured");
        }
