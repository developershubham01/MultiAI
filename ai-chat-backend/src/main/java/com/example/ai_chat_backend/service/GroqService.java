...
public class GroqService implements AIService {
    ...
    @Value("${ai.providers.groq.base-url:https://api.groq.com/openai/v1}")
    private String baseUrl;

    @Value("${ai.providers.groq.model:llama3-8b-8192}")
    private String model;

    @Value("${ai.providers.groq.enabled:true}")
    ...
    public ChatResponse chat(ChatRequest request) {
        ...
        if (!enabled || apiKey == null || apiKey.isEmpty()) {
            throw new RuntimeException("Groq service is not configured");
        }
